...
export class RecordsService {
    ...
    postRecordToServer(record: RecordsEntity): Observable<RecordsEntity> {
        return this.http.post<RecordsEntity>(this.recordsUrl, record, this.httpOptions).pipe(
            tap((newRecord: RecordsEntity) => console.log(`Added record id=${newRecord.id}`)),
            catchError((err) => {
    ...
    genId(): number {
        return this.storeRecords.length > 0 ? Math.max(...this.storeRecords.map((record) => record.id)) + 1 : 1;
    }
}
